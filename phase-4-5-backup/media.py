...
def _motion_filter(scene, frames: int) -> str:
...
def _render_scene_clip(
    ...
    filter_graph = _motion_filter(scene, frames)
...
Style: Dynamic,Arial,78,&H00FFFFFF,&H00FFFFFF,&H00101010,&H96000000,-1,0,0,0,100,100,0,0,3,3,0,2,80,80,360,1
Style: Hook,Arial,92,&H00FFFFFF,&H00FFFFFF,&H00101010,&HA0000000,-1,0,0,0,100,100,0,0,3,4,0,2,70,70,330,1
...
"""
    ...
    for scene in storyboard.scenes:
        phrases = _split_into_phrases(scene.narration, target_words=4)
        if not phrases:
        ...
        for index, (phrase, weight) in enumerate(zip(phrases, weights)):
            ...
            style = "Hook" if is_hook else "Dynamic"
            animation = (
...
def render_video(
    ...
    clips = []
    for scene, image_path in zip(storyboard.scenes, images):
