...
app = FastAPI(title="Mind Frontier Studio", version="10.0.0")
...
def content_plan(limit: int = 5, category: str | None = None):
    ideas = get_recommendations(limit=limit, category=category)
    write_content_roadmap(ROOT, limit=10)
    return {
...
def dashboard_project(project_id: str):
    try:
        return get_project(PROJECTS_DIR, project_id)
...
def create_project(request: ProjectRequest):
    try:
        ...
        save_beat_map(folder, narrative_beats)
